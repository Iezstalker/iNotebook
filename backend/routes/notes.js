...
router.post('/addnote',fetchuser, [
  ...
  // Route4.: Update an existing Note using : PUT "/api/notes/deletenote". login required
  router.delete('/deletenote/:id',fetchuser, async(req,res) =>{
    // const {title, description, tag} =req.body;

    //Delete a note by finding the note
    try{
    let note = await Note.findById(req.params.id)
    if(!note) {
    ...
    //Allow user deletion only if he owns this note
    if(note.user.toString() !== req.user.id){
      return res.status(401).send("Note Allowed");
    }

    note = await Note.findByIdAndDelete(req.params.id)
    res.json({"Success" : `Note ${req.user.id} has been deleted`})
  }catch(error){
    console.error(error.message)
    res.status(500).send("Internal Server Error")
  }
